...
class App(object):
	...
	def doConversion(self):
		for x in range(self._args.start,self._args.finish+1):
			os.system('convert -background {2} -fill {3} -font {1} -pointsize {4} label:"{0}" "{5}{0}.png"'.format(x, self._args.font, self._args.background, self._args.foreground, self._args.pointsize,
				self._args.prefix))
			pass		
