...
var Model = function (name, func) {
    ...
    var model = function (attributes) {
        this.attributes = Model.Utils.extend({
        ...
        if (Model.Utils.isFunction(this.initialize))
            this.initialize()
    }
    ...
    if (Model.Utils.isFunction(func))
        func.call(model, model, model.prototype)
...
Model.ClassMethods = {
    add: function (model) {
        ...
        if (Model.Utils.inArray(this.collection,
                                model) === -1 && !(id && this.find(id))) {
    ...
    detect: function (func) {
        var all = this.all(), model

        for (var i = 0, length = all.length; i < length; i++) {
            model = all[i]
            if (func.call(model, model, i))
                return model
        }
    },
    ...
    select: function (func, context) {
        var all = this.all(), selected = [], model

        for (var i = 0, length = all.length; i < length; i++) {
            model = all[i]
            if (func.call(context || model, model, i, all))
                selected.push(model)
        }
...
Model.InstanceMethods = {
    ...
    callPersistMethod: function (method, callback) {
        ...
        var wrappedCallback = function (success) {
            ...
            if (callback)
                value = callback.apply(self, arguments)
...
Model.REST = function (klass, resource, methods) {
    ...
    return Model.Utils.extend({
                                  path: function (model) {
                                      var path = resource
                                      jQuery.each(resource_param_names,
                                                  function (i, param) {
                                                      path = path.replace(
                                                                  ":" + param,
                                  ...
                                  params: function (model) {
                                      ...
                                      if (model) {
                                          ...
                                          params[model.constructor._name.toLowerCase(
                                                     )] = attributes
                                      } else {
                                          params = null
                                      }
                                      if (jQuery.ajaxSettings.data) {
                                          params = Model.Utils.extend({
                                  ...
                                  update: function (model, callback) {
                                      return this.xhr('PUT',
                                                      this.update_path(model),
                                                      model, callback)
                                  },
                                  update_path: function (model) {
                                      return [this.path(model), model.id(
                                                  )].join('/')
                                  },
                                  xhr: function (method, url, model, callback) {
                                      var self = this
                                      var data = method == 'GET' ? undefined : this.params(
                                                                       model)
                                  ...
                                  xhrComplete: function (xhr, textStatus, model, callback) {
                                      ...
                                      var data = Model.REST.parseResponseData(
                                                  xhr)
...
Model.REST.parseResponseData = function (xhr) {
    try {
        return /\S/.test(xhr.responseText) ? jQuery.parseJSON(
...
Model.Base = (function () {
    function Base() {}
    Base.prototype = Model.Utils.extend({
